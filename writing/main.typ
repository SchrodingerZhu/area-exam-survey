...
- @RC-Revisit continues on exploring the solutions but focusing on some newly developed methods based reuse analysis. This section will discuss runtime facilities and static analysis required by reuse analysis. Besides solving the inplace update problem, it also explains why reference-counting (RC) based reuse runtime enables a straightforward way to wrap imperative data structures into functional programs. Program patterns favored by reuse analysis will also be covered.
...
The first limitation also applies to some other works like @SISALSA. @SISALSA has a sophisticated system to optimize functional languages with streaming and iterations. As such, it also focus on a fixed set of operations that are important for streaming and iterations.
...
return xs' 
```
One may have noticed that we have consecutive `inc/dec` operations. Why not permeate them across control flow and cancel them with each other? This is demonstrated in the following code:
