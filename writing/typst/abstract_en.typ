#let abstract_en() = {
  ...
  text[
    This work examines prior research on the design and implementation that solves inplace update problems of functional programming languages. The survey covers common design patterns in functional languages that impose performance penalty due to immutability. To find answers to such issue, both traditional static analysis and newly developed RC-oriented optimizations are studied and carefully compared. The survey also overviews some potential future improvement that can be implemented in addition to existing reuse analysis framework.
  ]
